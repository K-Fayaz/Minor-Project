...
# starting coordinates for pedestriens
ped_x = {'right':[20,20],'down':[580,760],'left':[1350,1350],'up':[580,768]}
ped_y = {'right':[330,510],'down':[20,20],'left':[323,503],'up':[750,750]}

# speed of the pedestrien
ped_speed = 1
...
class Person(pygame.sprite.Sprite):
    def __init__(self,direction_number,side):
        pygame.sprite.Sprite.__init__(self)
        self.side = side
        self.direction_number = direction_number
        direction = directionNumbers[direction_number]
        self.x = ped_x[direction][side]
    ...
    def move(self):
        # if the direction is right(person is moving towards right) than incrase his x coord by 1
        if directionNumbers[self.direction_number] == 'right':
            self.x += ped_speed
        elif directionNumbers[self.direction_number] == 'left':
            self.x -= ped_speed
        elif directionNumbers[self.direction_number] == 'down':
            self.y += ped_speed
        else:
            self.y -= ped_speed
...
# generating Pedestriens in the simulation

def generatePedestrian():
    while(True):
        # find the direction
        ...
        # find the side of the direction
